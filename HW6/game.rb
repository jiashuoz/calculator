    ...
    def delta_attr(deltas = {})
        deltas.each do |attr, delta|
            if @attributes.has_key? attr
                ...
                # Edition
                print "The ", @name, "'s ", attr, " now is ", @attributes[attr].to_s, $/
            end
        end
    end
    ...
    def delta_attr(deltas = {})
        deltas.each do |attr, delta|
            if (@attributes.has_key? attr)
                ...
                print " by ", delta, ".", $/
                # Edition
                print "Your ", attr, " now is ", @attributes[attr].to_s, $/
            end
        end
    end
...
# A thing with a use.
class Item < Thing
    # Edition: changed the order of arguments world_text and detail text
    def initialize(name, brief_text, world_text = nil, detail_text = nil)
...
class Location
    attr_reader :name, :desc, :things
    # Edition: make doors accessor, easier to network location in game
    attr_accessor :doors
    ...
    def do(args = [])
        if (args.length < 1)
            ...
            if thing.is_a? Monster
                ...
                # Edition start
                # Monster should die if HP is 0
                if thing.attributes['hp'] <= 0
                    puts "The Monster has been killed."
                    Game.location.things.delete(thing.name)
                end
                thing.attack(Game.player)
                if Game.player.attributes['hp'] <= 0
